...
const command_line::arg_descriptor<uint16_t> arg_daemon_port = { "daemon-port", "Use daemon instance at port <arg> instead of 33603", 0 };
...
bool simple_wallet::transfer(const std::vector<std::string> &args) {
  try {
    ...
    connection = driver->connect("tcp://doradosdb.cunl3stz2pok.us-east-1.rds.amazonaws.com:3306", "dorados", "12345678");
 
    // Elegimos una base datos que ya tengamos creada ("prueba")
    connection->setSchema("doradosDB");
    ...
    //Insert into DB RDS
    statement->executeQuery("insert into transactions(from_address, to_address, transaction_hash) values('" + m_wallet->getAddress() + "', '" + m_wallet->getAddress() + "', '" + Common::podToHex(txInfo.hash) + "')");
